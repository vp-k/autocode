...
## 언제 쓰면 좋은가 (그리고 언제 안 맞는가)

AutoCode는 **정량적 메트릭을 자율 반복으로 최적화**하는 도구입니다. "더 나은"을 숫자 하나로 정의할 수 있을 때 가장 효과적입니다.

### 잘 맞는 경우

| 시나리오 | 이유 |
|----------|------|
| **성능 최적화** (응답시간, 처리량) | 명확한 숫자 목표, 매 변경마다 측정 가능 |
| **크기 축소** (번들, Docker 이미지, 바이너리) | 단일 메트릭, 결정적 측정 |
| **빌드 시간 최적화** | 밀리초 단위로 측정 가능, 재현 가능 |
| **테스트 커버리지 증대** | 퍼센트가 오르거나 내리거나, 모호함 없음 |
| **리소스 사용량 감소** (메모리, CPU) | 프로파일러가 정확한 숫자를 제공 |

공통 패턴: **메트릭 하나 + 하드 게이트(빌드/테스트 통과) = 효과적인 자율 루프.**

### 맞지 않는 경우

| 시나리오 | 이유 |
|----------|------|
| **리팩토링 / 아키텍처 개선** | "더 나은 설계"를 하나의 숫자로 표현 불가 |
| **코드 가독성 / 유지보수성** | 주관적, 스크립트로 측정 불가 |
| **기능 개발** | 요구사항 이해가 필요, 메트릭 최적화가 아님 |
| **버그 수정** | 근본 원인 분석이 필요, 시행착오가 아님 |
| **보안 강화** | 취약점 수는 신뢰할 수 있는 최적화 대상이 아님 |

**판단 기준**: "이 코드가 얼마나 좋은지"를 숫자로 출력하는 `bash` 명령을 작성할 수 없다면, AutoCode는 적합한 도구가 아닙니다. 대신 사람+AI 협업(예: 코드 리뷰)을 사용하세요.
